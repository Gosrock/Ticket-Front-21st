(() => {
   ...
   const sceneInfo = [
      {//0
         ...
         objs: {
            ...
            messageA: document.querySelector('#scroll-section-0 .performance-info.a'),
            messageB: document.querySelector('#scroll-section-0 .performance-info.b'),
            messageC: document.querySelector('#scroll-section-0 .performance-info.c'),
         },
         values: {
            //등장 투명도
   ...
   function playAnimaition(){
      ...
      switch (currentScene){
         case 0:
            ...
            const message_opacity_out = calcValues(values.message_opacity_out, currentYOffset);
            if(scrollRatio <= 0.22){
         ...
         case 1:
            break;
         case 2:
            const mobileA_opacity_in = calcValues(values.mobileA_opacity_in, currentYOffset);
            const mobileA_opacity_out = calcValues(values.mobileA_opacity_out, currentYOffset);
            if(scrollRatio <= 0.22 ){
               objs.mobileA.style.opacity = mobileA_opacity_in;
               objs.mobileA.style.transform = `translate3d(0, ${calcValues(values.mobileA_translateY_in, currentYOffset)}%, 0)`;
            }
            break;
         case 3:
            break;
   ...
   function scrollLoop(){
      ...
      if(yOffset > prevScrollHeight + sceneInfo[currentScene].scrollHeight){
         ...
         enterNewScene = true;
      if(currentScene === 0){return;} // 모바일에서 바운스효과로 나타나는 버그 방지
         currentScene--;
      }
      document.body.setAttribute('id', `show-scene-${currentScene}`);
